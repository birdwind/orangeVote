package com.orange.orange_vote.base.swagger;
 ...
 *
 * @see com.dlin.dlcooperative.base.swagger.CustomizeModelAttributeParameterExpander#getBeanPropertyNames(Class)
...
public class CustomizeModelAttributeParameterExpander extends ModelAttributeParameterExpander {
    ...
    public List<Parameter> expand(ExpansionContext context) {
        ...
        final AlternateTypeProvider alternateTypeProvider =
            context.getDocumentationContext().getAlternateTypeProvider();

        FluentIterable<ModelAttributeField> attributes =
            allModelAttributes(propertyLookupByGetter, getters, fieldsByName, alternateTypeProvider);

        FluentIterable<ModelAttributeField> expendables =
            attributes.filter(not(simpleType())).filter(not(recursiveType(context)));
        ...
        FluentIterable<ModelAttributeField> collectionTypes =
            attributes.filter(and(isCollection(), not(recursiveCollectionItemType(context.getParamType()))));
    ...
    private FluentIterable<ModelAttributeField> allModelAttributes(
        ...
        FluentIterable<ModelAttributeField> modelAttributesFromGetters =
            from(getters).transform(toModelAttributeField(fieldsByName, propertyLookupByGetter, alternateTypeProvider));

        FluentIterable<ModelAttributeField> modelAttributesFromFields =
            from(fieldsByName.values()).filter(publicFields()).transform(toModelAttributeField(alternateTypeProvider));
        ...
        try {
            ...
            for (PropertyDescriptor descriptor : descriptors) {
                ...
                if (field != null) {
                    ...
                    JsonIgnore ignoreSwaggerParameter =
                        field.getDeclaredAnnotation(JsonIgnore.class);
