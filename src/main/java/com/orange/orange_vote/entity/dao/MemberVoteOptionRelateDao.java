...
public interface MemberVoteOptionRelateDao extends BaseRepository<MemberVoteOptionRelate, Integer> {
    ...
    @Query(value = "SELECT mvor FROM MemberVoteOptionRelate mvor "
        + "JOIN VoteOption vo ON vo.voteOptionId = mvor.voteOption.voteOptionId "
        + "WHERE vo.vote.voteId = ?1 AND mvor.status = true AND mvor.member = ?2")
    Optional<List<MemberVoteOptionRelate>> findVoteAlreadyByVoteId(Integer voteId, Member memberId);
