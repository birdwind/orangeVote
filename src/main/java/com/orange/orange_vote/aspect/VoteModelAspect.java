...
public class VoteModelAspect extends AbstractAuthModelAspect<Vote> {
    ...
    @Override
    protected Vote authenticate(String source) throws Throwable {
        Member member = SystemUser.getMember();
        return voteService.getVoteByVoteUuidAndMemberIdAndCreatorId(source, member.getMemberId())
            .orElseThrow(() -> new EntityNotFoundException("voteUuid", VoteErrorConstants.VOTE_NOT_FOUND));
    }
}
